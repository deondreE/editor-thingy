...
5. If project memory still matters, run `brain find editor-thingy` or `brain search "editor-thingy <task>"`.
...
- Use `nix-shell` for the intended dev environment.
- Repo verification baseline:
  - `odin check .`
  - `odin run .`
- Fresh environments may need shader compilation and STB static library compilation before the app launches.
